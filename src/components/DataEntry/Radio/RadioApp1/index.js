...
export class RadioApp1 extends React.Component {
    state = {
        value: 0,
        value2: 0,
        value3: 0,
    ...
    render() {
        ...
        return (<div>
            ...
            <List renderHeader={() => 'Disabled'}>
                {data.map(i => (
                    <RadioItem key={i.value} checked={value3 === i.value} onChange={() => this.onChange3(i.value)}
                               disabled>
                        {i.label}
                    </RadioItem>
                ))}
            ...
            <List>
                {data2.map(i => (
                    <RadioItem key={i.value} checked={value4 === i.value} onChange={() => this.onChange4(i.value)}
                               disabled>
                        {i.label}<List.Item.Brief>{i.extra}</List.Item.Brief>
                    </RadioItem>
                ))}
            ...
            <Flex style={{padding: '15px'}}>
                <Flex.Item style={{padding: '15px 0', color: '#888', flex: 'none'}}>Radio demo(dustomized
                    style)</Flex.Item>
                <Flex.Item>
